...
class ExcelBackupService {
  ...
  static Future<String> exportFullBackup({void Function(double progress)? onProgress}) async {
    ...
    _addRow(summarySheet, ['Total Borrowers', borrowers.where((b) => (b['is_deleted'] as int? ?? 0) == 0 && (b['is_dummy'] as int? ?? 0) == 0).length.toString()]);
    ...
    final themePrefs = await ProfilePrefs.getInstance();
  ...
  static Future<void> importBackup(
    ...
    await db.transaction((txn) async {
      if (!merge) {
        debugPrint('[Restore] Wiping local DB...');
        final tables = ['service_costs', 'investments', 'expenses', 'payments', 'loans', 'borrowers'];
        for (final t in tables) {
          final countRes = await txn.rawQuery("SELECT COUNT(*) as count FROM $t");
          debugPrint('[Restore] $t count before wipe: ${countRes.first['count']}');
          
          await txn.delete(t);
          
          // Explicitly reset auto-increment to ensure determinism across platforms
          try {
            await txn.rawDelete("DELETE FROM sqlite_sequence WHERE name = ?", [t]);
          } catch (e) {
            // sqlite_sequence might not exist if no auto-increments have ever been used
          }
          
          final countResAfter = await txn.rawQuery("SELECT COUNT(*) as count FROM $t");
          debugPrint('[Restore] $t count after wipe: ${countResAfter.first['count']}');
        }
      }
      ...
      if (bSheet != null && bSheet.maxRows > 1) {
        ...
        for (int i = 1; i < bSheet.maxRows; i++) {
          ...
          if (merge) {
            ...
            // Full restore: strict 1:1 mapping. Reject corrupted duplicate rows.
            try {
              final newId = await txn.insert('borrowers', sanitized);
              borrowerIdMap[oldId] = newId;
            } catch (e) {
              if (e.toString().contains('UNIQUE constraint')) {
                throw Exception('Critical Restore Error: A wiped database rejected a new borrower insertion (Duplicate borrower_code: ${sanitized['borrower_code']}). This indicates stale restore state or corrupted backup.');
              }
              rethrow;
            }
          }
      ...
      if (lSheet != null && lSheet.maxRows > 1) {
        ...
        for (int i = 1; i < lSheet.maxRows; i++) {
          ...
          final newBorrowerId = borrowerIdMap[oldBorrowerId];
          if (newBorrowerId == null) {
            debugPrint('Restore: Skipping loan (Backup ID: $oldId) because parent borrower (Backup ID: $oldBorrowerId) was not mapped.');
            continue; // Orphaned
          }
          sanitized['borrower_id'] = newBorrowerId;

          // Verify borrower exists locally to prevent FOREIGN KEY constraint failure
          final validBorrower = await txn.query('borrowers', columns: ['id'], where: 'id = ?', whereArgs: [sanitized['borrower_id']]);
          if (validBorrower.isEmpty) {
            debugPrint('Restore: Skipping loan (Backup ID: $oldId) because mapped borrower ${sanitized['borrower_id']} does not exist locally.');
            continue;
          }
      ...
      if (pSheet != null && pSheet.maxRows > 1) {
        ...
        for (int i = 1; i < pSheet.maxRows; i++) {
          ...
          final newLoanId = loanIdMap[oldLoanId];
          if (newLoanId == null) {
            debugPrint('Restore: Skipping payment because parent loan (Backup ID: $oldLoanId) was not mapped.');
            continue;
          }
          sanitized['loan_id'] = newLoanId;

          // Verify loan exists locally to prevent FOREIGN KEY constraint failure
          final validLoan = await txn.query('loans', columns: ['id'], where: 'id = ?', whereArgs: [sanitized['loan_id']]);
          if (validLoan.isEmpty) {
            debugPrint('Restore: Skipping payment because mapped loan ${sanitized['loan_id']} does not exist locally.');
            continue;
          }
      ...
      // Print Phase 7 post-restore stats
      debugPrint('[Restore] === Restore Summary ===');
      final pCounts = {
        'borrowers': await txn.rawQuery('SELECT COUNT(*) as count FROM borrowers'),
        'loans': await txn.rawQuery('SELECT COUNT(*) as count FROM loans'),
        'payments': await txn.rawQuery('SELECT COUNT(*) as count FROM payments'),
        'expenses': await txn.rawQuery('SELECT COUNT(*) as count FROM expenses'),
        'investments': await txn.rawQuery('SELECT COUNT(*) as count FROM investments'),
        'service_costs': await txn.rawQuery('SELECT COUNT(*) as count FROM service_costs'),
      };
      for (final entry in pCounts.entries) {
        debugPrint('[Restore] ${entry.key}: ${entry.value.first['count']} rows');
      }
      debugPrint('[Restore] =======================');
    });
  ...
  static Map<String, dynamic> _rowToMap(List<String> headers, List<Data?> row) {
    ...
    for (int i = 0; i < headers.length; i++) {
      if (i < row.length && row[i] != null && row[i]!.value != null) {
        ...
        if (['id', 'borrower_id', 'loan_id', 'installment_days', 'updated_at', 'created_at', 'is_synced', 'is_deleted', 'is_dummy', 'timestamp'].contains(headers[i])) {

...
# I need to locate the block starting with:
#                 IconButton(
#                   onPressed: () async {
#                     final messenger = ScaffoldMessenger.of(context);
#                     final provider = context.read<LoanProvider>();
#                     final loans = await provider.getLoans(b.id ?? 0);

# And ending before:
#                   icon: const Icon(LucideIcons.share2, color: AppColors.accent, size: 20),

# Let's extract the full block so I can safely replace it.
pattern = r'(IconButton\(\s*onPressed:\s*\(\)\s*async\s*\{\s*final\s*messenger\s*=\s*ScaffoldMessenger\.of\(context\);\s*final\s*provider\s*=\s*context\.read<LoanProvider>\(\);\s*final\s*loans\s*=\s*await\s*provider\.getLoans\(b\.id\s*\?\?\s*0\);.*?\}\s*,\s*icon:\s*const\s*Icon\(LucideIcons\.share2.*?,\s*size:\s*20\),\s*\))'

match = re.search(pattern, content, flags=re.DOTALL)
if match:
    old_block = match.group(1)
    
    new_block = """IconButton(
                  onPressed: () async {
                    final messenger = ScaffoldMessenger.of(context);
                    final provider = context.read<LoanProvider>();
                    ...
                    // Calculate correct balance. If sharing a single loan, use that loan's balance. 
                    // If combined, use borrower's total balance.
                    ...
                    try {
                      ...
                      if (Platform.isAndroid) {
                        ...
                        if (Platform.isWindows) {
                          try {
                            await Process.run('powershell.exe', ['-NoProfile', '-Command', f"Set-Clipboard -Path '{filePath}'"]);
                          } catch (_) {}
                          messenger.showSnackBar(
                            SnackBar(
